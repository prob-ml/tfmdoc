...
sed "s/\\.0//g" $INPATH$pharm | cut -d "," -f 1,5,8,15,20,26,27| sed 's/ /_/g' | sed "s/\"//g" | head -1 > $OUTFILE
...
do
    ...
    do
    	sed "s/\\.0//g" $INPATH$pharm | cut -d "," -f 1,5,8,15,20,26,27 | egrep -i $pharm_drug | sed 's/ /_/g' | sed "s/\"//g" >> $OUTFILE
    done
done
